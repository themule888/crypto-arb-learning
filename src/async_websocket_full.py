...
async def monitor_continuously(coin='bitcoin', interval=30):
    """Monitor prices continuously via HTTP and log to CSV"""
    ...
    try:
        while True:
            ...
            print(f'\n⏳ Next HTTP check in {interval} seconds...')
            print('=' * 60)
            await asyncio.sleep(interval)

    except KeyboardInterrupt:
        print(f'\n\n⛔ HTTP monitoring stopped by user')
            ...
            async with session.get('https://api.coingecko.com/api/v3/simple/price', params=params) as response:
                if response.status == 200:
                    ...
                    if btc_ws and eth_ws:
                        ...
                        print(f'   BTC - HTTP: ${btc_http:,.2f} | WebSocket: ${btc_ws:,.2f} | Diff: ${btc_diff:.2f}')
                        print(f'   ETH - HTTP: ${eth_http:,.2f} | WebSocket: ${eth_ws:,.2f} | Diff: ${eth_diff:.2f}\n')
...
async def main():
    ...
    try:
        ...
        await asyncio.gather(
            ...
            monitor_continuously(coin='bitcoin', interval=30),
